  ...
  async function oldAppResponseHandler(app) {
    try {
      ...
      if (typeof app === "string") {
        const dbApp = await G_Apps.findOne({ _id: app }).select("published").lean();
        try {
          if (dbApp.published) {
      ...
      if (typeof app === "object") {
        G_Apps.findOne({ _id: app.appId }).lean().then((dbApp) => {
          if (config.timeline) {
...
function IOSApp(){
  ...
  async function oldAppResponseHandler(app) {
    try {
      // if the app was removed
      if (typeof app === "string") {
        Ios_Apps.findOne({ _id: app }).select("published").lean()
          .then((dbApp) => {
      ...
      // if the app exist
      if (typeof app === "object") {
        Ios_Apps.findOne({ _id: app.id }).lean().then((dbApp) => {
            updateTheApp(app, dbApp);
